...
//    var sortedCards:[Datum]{return cardlist.data}
    ...
    @State private var deck: [Datum] = []
    @State private var isCardAdded = false

    
    private func addCardToDeck(card: Datum) {
            if !deck.contains(where: { $0.id == card.id }) {
                deck.append(card)
                isCardAdded = true // Tampilkan modal/pesan konfirmasi
            }
        }
    var body: some View {
        TabView {
...
//            NavigationView {
//                        List(cardlist.data.indices, id: \.self) { index in
//                            NavigationLink(destination: CardDetailView(cards: cardlist.data, currentIndex: index, addToDeck: addCardToDeck)) {
//                                Text(cardlist.data[index].name)
//                            }
//                        }
//                        .alert(isPresented: $isCardAdded) {
//                            Alert(title: Text("Card Added"), message: Text("This card has been added to your deck."), dismissButton: .default(Text("OK")))
//                        }
//                    }
                    ...
                    if(isFilterPresented==false){
                        LazyVGrid(columns: columns, spacing: 20) {
                            ForEach(sortedCards.indices, id: \.self) { index in
                                NavigationLink(destination: CardDetailView(cards: sortedCards, currentIndex: index, addToDeck: addCardToDeck)){
                                    VStack {
                                        CardView(card: sortedCards[index]) // Menggunakan CardView sebagai subview
                                        Text(sortedCards[index].name)
                                            .bold()
                                            .lineLimit(2)
            ...
            NavigationView {
                       VStack {
                           // Tampilkan daftar kartu atau konten lain
                           // ...
                           DeckView(deck: deck)
    ...
    var body: some View {
        ZStack{
            ScrollView(.vertical, showsIndicators: false) {
                ...
                VStack (spacing:0){
                    ...
                    AsyncImage(url: URL(string: cards[currentIndex].imageUris.artCrop)) { image in
                        image
                            .resizable()
                            .scaledToFit()
                            .frame(alignment: .top)
                    ...
                    .onTapGesture {
                        // Logika untuk menampilkan popup, misalnya pada tombol atau event tertentu
                        isPopupPresented = true
                    }
                    //
                    ...
                    HStack{
                        VStack(alignment: .leading){
                            ...
                            Text(cards[currentIndex].typeLine)
                                .multilineTextAlignment(.leading)
                                .bold()
                        ...
                        Spacer()
                        VStack{
                            ManaCostView(manaCost: cards[currentIndex].manaCost)
                                .font(.subheadline)
                            Text("")
                    ...
                    HStack(spacing:0){
                        Text(cards[currentIndex].oracleText).background(Color(.systemGray6)).padding().cornerRadius(4) .padding(.horizontal, 4)
                            .padding(.vertical, 2)
                    ...
                    if selectedButton == "Button1" {
                        VStack{
                            HStack{
                                CardViewKecil(card: cards[currentIndex])
                                    .onTapGesture {
                                        isPopupPresented = true
                                    }
                                Spacer()
                                HStack{
                                    Spacer()
                                    if (cards[currentIndex].nonfoil==true){
                                        VStack{
                                            Text("Normal").bold()
                                            if let usdPrice = cards[currentIndex].prices.usd {
                                                Text("$\(usdPrice)").bold()
                                            }
                                        }
                                    }
                                    Spacer()
                                    if (cards[currentIndex].foil==true){
                                        VStack{
                                            Text("Foil").bold()
                                                .foregroundColor(.orange)
                                            if let usdPrice = cards[currentIndex].prices.usdFoil {
                                                Text("$\(usdPrice)").bold()
                                            }
                                        }
                                    }
                        ...
                        LegalitiesView(legalities: cards[currentIndex].legalities)
                            .padding()
                        // Customisasi tampilan lebih lanjut di sini
                    }
            ...
            .navigationBarTitle(showTitle ? cards[currentIndex].name : "", displayMode: .inline)
            ...
            if isPopupPresented {
                PopupView(isPopupPresented: $isPopupPresented, card: cards[currentIndex])
                    .animation(.easeInOut, value: isPopupPresented)
            }
            
            HStack {
                            // Tombol kiri hanya ditampilkan jika currentIndex > 0
                            if currentIndex > 0 {
                                Button(action: {
                            ...
                            // Tombol kanan hanya ditampilkan jika currentIndex < jumlah kartu - 1
                    ...
                    DragGesture()
                        .onEnded { value in
                            ...
                            if value.translation.width > dragThreshold {
                                // Swipe ke kanan, pindah ke kartu sebelumnya
                                currentIndex = max(currentIndex - 1, 0)
                            } else if value.translation.width < -dragThreshold {
                                // Swipe ke kiri, pindah ke kartu selanjutnya
                                currentIndex = min(currentIndex + 1, cards.count - 1)
                            }
                        }
                )
        .navigationBarBackButtonHidden(true)
                .navigationBarItems(leading: Button(action: {
                    ...
                    Image(systemName: "chevron.left") // Ikon panah tanpa batang
                        .aspectRatio(contentMode: .fit)
                        .foregroundColor(.white) // Atur warna jika diperlukan
                        .bold()
...
struct ColoredSearchBar: UIViewRepresentable {
    ...
    func makeUIView(context: Context) -> UISearchBar {
        ...
        searchBar.barTintColor = UIColor.white // Ubah warna sesuai keinginan
        searchBar.tintColor = UIColor.white // Ubah warna teks dan ikon
        return searchBar
    }
...
struct DeckView: View {
    @State  var deck: [Datum] // Deck berisi array dari kartu (Datum)

    var body: some View {
        NavigationView {
            List {  ForEach(deck.indices, id: \.self) { index in
                VStack{
                    HStack{
                        
                        CardViewDeck(card: deck[index])
                        Spacer()
                        VStack{
                            Text(deck[index].name)
                                .bold()
                                .lineLimit(2)
                                .fixedSize(horizontal: false, vertical: true)
                                .font(.title2)
                            ...
                            ManaCostView(manaCost: deck[index].manaCost)
                                .font(.subheadline)
                        }
                    }
                    
                    Button(action: {
                                            removeFromDeck(at: index)
                                        }) {
    ...
    private func removeFromDeck(at index: Int) {
